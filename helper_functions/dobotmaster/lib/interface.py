...
######################################################################################################################
# own implmented functions (more further down)
######################################################################################################################
    def clear_alarms(self):
        """
        Löscht bestehende Fehlermeldungen und Alarme am Dobot.
        """
        request = Message(
            header=[0xAA, 0xAA],
            length=2,
    ...
    def home(self):
        """
        Führt das Homing des Dobots aus (fährt zu Nullpunktpositionen).
        """
        # Message ID 31 = Homing Command
        request = Message(
            header=[0xAA, 0xAA],
            length=2,
            id=31,
    ...
    def wait_until_done(self, target_index=None):
        """
        Wartet, bis der Dobot alle Befehle bis zum angegebenen Index abgearbeitet hat.
        Wenn kein Index angegeben ist, wartet er auf das Ende der aktuellen Queue.
        """
        sleep(0.1)  # kurz warten, damit Queue-Index aktualisiert wird

        if target_index is None:
            target_index = self.get_current_queue_index()
...
#############################################################################################
    ...
    ####################### Own implementation #######################
    def set_motor_speed_direct(self, index=0, speed=500.0, enable=1, queue=False):
        """
        Steuert den Conveyor Belt manuell über Message ID 135 im Dobot-Protokoll.
        Dies ist eine manuelle Alternative zu set_extended_motor_velocity(), um volle Kontrolle zu haben.
        """
        payload = list(pack('<BBf', index, enable, float(speed)))
        msg = Message([0xAA, 0xAA], 2, 135, True, queue, [], direction='out', raw=True)
        msg.raw_params = payload
        print(f"Sending message: {msg.package()}")
        return self.send(msg)

    def set_conveyor_speed(self, speed: float, interface: int = 0, enable: bool = True):
        """
        Setzt die Geschwindigkeit des Conveyor Belt über Message ID 135 (Stepper Motor).
        Nachbau der dobotapi-Logik.
        """
        speed_int = int(speed)
        params = bytearray()
        params.append(interface)                   # 0 für normalen Anschluss
        params.append(1 if enable else 0)          # Motor einschalten
        params.extend(pack("i", speed_int))        # 4 Byte signed int (little endian)

        msg = Message([0xAA, 0xAA], 2, 135, True, False, list(params), direction='out', raw=True)
        msg.ctrl = 0x03
        return self.send(msg)
    def set_conveyor_raw(self, speed: int, interface: int = 0, enable: bool = True):
        """
        Sendet exakt denselben Conveyor-Befehl wie dobotapi – ohne Message-Klasse.
        """
        id = 135
        ctrl = 3
        params = bytearray()
        ...
        params.extend(pack('<i', speed))  # signed int, little endian
    
        length = 2 + len(params)  # id + ctrl + params
        checksum = (id + ctrl + sum(params)) % 256
        checksum = (256 - checksum) % 256
    
        command = bytearray([0xAA, 0xAA, length, id, ctrl])
        command.extend(params)
        command.append(checksum)
    
        print("→ Sende Rohdaten:", list(command))  # zum Debuggen
        self.serial.write(command)

    # def get_ir_state(self) -> bool:
    #     """
    #     Gibt zurück, ob der IR-Sensor ein Objekt erkennt (True) oder nicht (False).
    #     """
    #     result = self.get_ir_switch(0)
    #     if isinstance(result, list) and len(result) > 0:
    #         return result[0] == 1
    #     return False  # Fallback, falls keine Daten oder unerwartetes Format
    def get_ir_state(self, port=4, index=1, version=1):
        """
        Gibt True zurück, wenn der IR-Sensor ein Objekt erkennt.
        Nachbau von dobotapi.get_ir()
        """
        from .message import Message
        from struct import unpack_from

        msg = Message([0xAA, 0xAA], 2, 138, False, False, [], direction='out', raw=True)
        msg.ctrl = 0x00  # Lesebefehl
        msg.raw_params = [port, index, version]

        self.send(msg)
        response = Message.read(self.serial)

        if response and response.raw_params and len(response.raw_params) > 0:
            raw = response.raw_params[0]
            print("IR full response:", list(response.raw_params))
            return raw == 1  # typischerweise 1 = erkannt, 0 = leer
        return False
    def close_col(self):
...
####################################################################################
    ...
    # Doesnt seem to be necessary, DOBOT only expects 1 byte of input (enable)
    ...
    #     return self.send(request)
    def set_ir_switch(self, enable, queue=True):
        request = Message([0xAA, 0xAA], 2, 138, True, queue, [enable], direction='out')
        return self.send(request)
